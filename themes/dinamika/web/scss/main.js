...
/**
   ...
   */
    ...
    $('.dropdown-cat .wrapper-dropdown__header').on("click", function(){
        $('.dropdown-cat .wrapper-dropdown__header').toggleClass('active');
        $('.dropdown-cat .wrapper-dropdown__body').toggleClass('active');
    });
    $(document).mouseup(function (e){
        if(!$('.dropdown-cat').is(e.target) // если клик был не по нашему блоку
            && $('.dropdown-cat').has(e.target).length === 0) { // и не по его дочерним элементам
            $('.dropdown-cat .wrapper-dropdown__header').removeClass('active');
            $('.dropdown-cat .wrapper-dropdown__body').removeClass('active'); // скрываем его
        }
    });

    /* выпадающее меню подбор оборудования*/
    $('.select-form .select-form__btn').on("click", function(){
        $('.select-form .select-form__btn').toggleClass('active');
        $('.select-form .select-form__body').toggleClass('active');
    });
    $(document).mouseup(function (e){
        if(!$('.select-form').is(e.target)
            && $('.select-form').has(e.target).length === 0) { 
            $('.select-form .select-form__btn').removeClass('active');
            $('.select-form .select-form__body').removeClass('active');
        }
    });
    ...
    */
    ...
    jQuery(document).ready(function($) {
        ...
        var beforeEnd = 500;
        var $imageSlider = $('.carouselSlider');
        $imageSlider.slick({
            fade: true,
            infinite: true,
    ...
    if($('div').hasClass('news-box')){
        $('.news-box').slick({
            ...
            arrows: true,
            dots: false,
            appendArrows: '.news-arrows .arrows',
            focusOnSelect: false,
            cssEase: 'cubic-bezier(0.7, 0, 0.3, 1)',
            touchThreshold: 100,
            speed: 700, 
