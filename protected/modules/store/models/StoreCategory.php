...
class StoreCategory extends \yupe\models\YModel
{
    ...
    public function relations()
    {
        return [
            ...
            'photos' => [self::HAS_MANY, 'CategoryImage', 'category_id'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'short_description' => Yii::t('StoreModule.store', 'Description'),
            'description' => Yii::t('StoreModule.store', 'Short description'),
    ...
    {
        if (Yii::app()->getRequest()->getPost('CategoryImage')) {
            foreach (Yii::app()->getRequest()->getPost('CategoryImage') as $key => $val) {
                $CategoryImage = CategoryImage::model()->findByPk($key);
                if (null === $CategoryImage) {
                }
                $CategoryImage->setAttributes($_POST['CategoryImage'][$key]);
                if (false === $CategoryImage->save()) {
                    Yii::app()->getUser()->setFlash(\yupe\widgets\YFlashMessages::ERROR_MESSAGE,
                        Yii::t('StoreModule.store', 'Error uploading some images...'));
                }
            }
        }
        foreach (CUploadedFile::getInstancesByName('CategoryImage') as $key => $image) {
            $CategoryImage = new CategoryImage();
            $CategoryImage->category_id = $this->id;
            $CategoryImage->attributes = $_POST['CategoryImage'][$key];
            $CategoryImage->addFileInstanceName('CategoryImage[' . $key . '][image]');
            $CategoryImage->save();
        }
    }
}
